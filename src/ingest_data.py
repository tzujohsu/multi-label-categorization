...
class ZipDataIngestor(DataIngestor):
    def ingest(self, file_path: str) -> pd.DataFrame:
        ...
        # Find the extracted JSON file (assuming there is one JSON file inside the zip)
        extracted_files = os.listdir("extracted_data")
        json_files = [f for f in extracted_files if f.endswith(".json")]

        if len(json_files) == 0:
            raise FileNotFoundError("No JSON file found in the extracted data.")
        if len(json_files) > 1:
            raise ValueError("Multiple JSON files found. Please specify which one to use.")

        # Read the JSON into a DataFrame
        json_file_path = os.path.join("extracted_data", json_files[0])
